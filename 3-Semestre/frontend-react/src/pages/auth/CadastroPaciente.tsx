    ...
    return (
        <div className="w-full animate-fade-in py-4">
            ...
            <div 
                ...
                aria-valuemax={3}
                aria-valuenow={step}
                aria-label={`Passo ${step} de 3 do cadastro`}
            >
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1 bg-brand-pot/30 -z-10" aria-hidden="true"></div>
                <div className={`absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-brand-orange transition-all duration-500 -z-10 ${step === 1 ? 'w-0' : step === 2 ? 'w-1/2' : 'w-full'}`} aria-hidden="true"></div>
                
                {[1, 2, 3].map((item) => (
                    <div 
                        key={item} 
                        className={`w-8 h-8 rounded-full flex items-center justify-center font-bold border-2 transition-colors ${step >= item ? 'bg-brand-orange border-brand-orange text-white' : 'bg-white border-brand-pot text-brown-light'}`}
